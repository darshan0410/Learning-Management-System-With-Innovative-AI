...
import { FaRobot, FaMicrophone, FaRegClock, FaMagic, FaUser, FaBriefcase, FaChartLine, FaFileUpload, FaChartBar, FaFileAlt, FaFilePdf, FaHistory, FaVideo, FaExclamationTriangle, FaArrowLeft } from "react-icons/fa";
...
const HomePage = () => {
  ...
  const handleFileUpload = async (event) => {
    ...
    try {
      const res = await axios.post(`${import.meta.env.VITE_BACKEND_URL || "http://localhost:5000"}` + "/api/interview/upload-resume", formData);
      if (res.data.success) {
  ...
  const analyzeResume = async () => {
    ...
    setAnalyzing(true);
    try {
      const res = await axios.post(`${import.meta.env.VITE_BACKEND_URL || "http://localhost:5000"}` + "/api/interview/extract-resume", { resumeText });
  ...
  const startInterview = async () => {
    setLoading(true);
    try {
      const res = await axios.post(`${import.meta.env.VITE_BACKEND_URL || "http://localhost:5000"}` + "/api/interview/generate-questions", {
      ...
      if (res.data.success && res.data.questions.length > 0) {
        navigate("/mockinterview/interview", {
          state: {
  ...
  return (
    <div className={`mi-container ${!showSetup ? "landing-mode" : ""}`}>
      <div style={{ width: '100%', maxWidth: '1100px', padding: '20px 20px 0 20px', display: 'flex' }}>
        <button 
          onClick={() => showSetup ? setShowSetup(false) : navigate("/")}
          style={{ 
      ...
      {!showSetup && (
        <div className="mi-landing fade-in">
          ...
          <div className="landing-actions">
            ...
            <button className="landing-btn-secondary" onClick={() => navigate("/mockinterview/history")}>View History</button>
          </div>
      ...
      {showSetup && (
        <div className="mi-setup fade-in mi-setup-card">
          <div className="setup-left">
            ...
            <div className="proctoring-info-box fade-in">
              <h4 className="proctoring-h4">
                <FaVideo color="#ef4444" style={{ marginRight: 6 }} />
                Proctoring & Integrity System
              </h4>
              <p className="proctoring-p">To mirror a real-world assessment, strict integrity rules apply:</p>
              <ul className="proctoring-ul">
                <li><FaExclamationTriangle color="#f59e0b" /> <strong>Camera Active:</strong> Live webcam proctoring is mandatory.</li>
                <li><FaExclamationTriangle color="#f59e0b" /> <strong>Absence Penalty:</strong> Stepping away from the camera throws a penalty.</li>
                <li><FaExclamationTriangle color="#f59e0b" /> <strong>No Devices:</strong> AI scanning detects mobile phones and books.</li>
                <li><FaExclamationTriangle color="#f59e0b" /> <strong>Tab Switching:</strong> Navigating away from the page is traced.</li>
              </ul>
              <div className="proctoring-fatal">Failure to comply with these rules or reaching 5 warnings will immediately terminate your session.</div>
            </div>
          </div>
