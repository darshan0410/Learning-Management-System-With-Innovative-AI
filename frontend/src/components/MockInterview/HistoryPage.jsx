...
const HistoryPage = () => {
  ...
  useEffect(() => {
    if (userData?._id) {
      axios.get(`${import.meta.env.VITE_BACKEND_URL || "http://localhost:5000"}/api/interview/history/${userData._id}`)
  ...
  const deleteSession = (id) => {
    axios.delete(`${import.meta.env.VITE_BACKEND_URL || "http://localhost:5000"}/api/interview/history/${id}`)
      .then(() => {
          ...
          <div className="dash-header">
            <div className="dash-header-left">
              ...
              <div>
                ...
                <p className="dash-subtitle">{selectedHistory.difficulty} • {selectedHistory.experience} • {new Date(selectedHistory.createdAt || selectedHistory.date).toLocaleDateString()}</p>
              </div>
            </div>
          </div>
  ...
  return (
    <div className="mi-container">
      <div className="iv-dashboard fade-in">
        <div className="dash-header">
          <div className="dash-header-left">
            <button className="dash-back" onClick={() => navigate("/mockinterview")}>
              <FaArrowLeft />
            </button>
            <div>
        ...
        {history.length === 0 ? (
          ...
          <div className="hist-list">
            {history.map((h) => {
              ...
              const dateStr = new Date(h.createdAt || h.date).toLocaleDateString("en-GB", { day: '2-digit', month: '2-digit', year: 'numeric' });
              return (
                <div key={h._id || h.id} className="nhc-card" onClick={() => setSelectedHistory(h)}>
                  ...
                  <div className="nhc-right">
                    ...
                    <div className="hist-info">
                      ...
                      <span className="hist-date">
                        {new Date(h.createdAt || h.date).toLocaleDateString("en-US", {
                  ...
                  <div className="hist-card-actions">
                    ...
                    <button className="hist-del-btn" onClick={(e) => { e.stopPropagation(); deleteSession(h._id || h.id); }} title="Delete">
                      <FaTrash size={12} />
                    </button>
                  </div>
                </div>
